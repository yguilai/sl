...
// Filter .
func Filter[T any](ps PseudoStream[T], fn Predicate[T]) PseudoStream[T] {
	s := make(chan T, 1)
	go func() {
		defer close(s)
		for t := range ps {
...
// Sort .
func Sort[T any](sources []T, less func(i, j int) bool) {
	// TODO: change implement
	sort.Slice(sources, less)
}
